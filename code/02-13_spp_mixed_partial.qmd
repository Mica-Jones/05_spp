...
spp_rcbd_mix_mod <- lmer(yield_kgha ~ krate_kgha*nrate_kgha +
                         (1|rep/krate_kgha),
                         data = spp_rcbd_dfw) # this line is to nest split plot effect inside block effect; R reads nests from largest to smallest. Always nest such that smallest plot variable is left out of random effect term. For example, if we were to add a P rate as the smallest split plot subdivision, you would add a *prate_kgha to the fixed effect term and make the random effect term "(1|rep/krate_kgha/nrate_kgha)"
...
```{r k rate means}
spp_rcbd_mix_means_krate <- emmeans(spp_rcbd_mix_mod, 
                                  ~krate_kgha)

spp_rcbd_mix_means_krate
```
...
```{r n rate means }
spp_rcbd_mix_means_nrate <- emmeans(spp_rcbd_mix_mod, 
                                  ~nrate_kgha)

spp_rcbd_mix_means_nrate
```
It is 209.  
